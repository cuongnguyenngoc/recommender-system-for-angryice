...
class BotTwitch:
    ...
    def __init__(self):
        ...
        self.describers = {} ### {userid1: {imgid1: {desid: "description"}, {imgid2: {desid: "description"}}, userid2:...}
        self.des_id = 0
        self.voters = {}
    ...
    ############################################################################
                ...
                if (user == self.OWNER) and (message == "!next\r"):
                    ...
                    if bool(self.describers):
                        self.des += 1
                    
                    des = {}
                    if (user in self.describers):
                        des = self.describers.get(user)
                    ...
                    if img_id in images:
                        des[img_id] = {self.des_id: description}
                        self.describers[user] = des

                        self.utils.writeVoteAndTagsData(user, message, images, self.game_session)
                    else:
                        self.sendMessage("There is no typed image_id shown on the screen. Please try again following the format.")
                    break
                elif (not self.isStopVotingSignal) and (user != self.OWNER) and message.startswith("#"):
                    print(user + " what is going on " + message)
                    self.utils.writeVoteData(user, message, artManager.get_images(), self.game_session)
                    break
                else:
