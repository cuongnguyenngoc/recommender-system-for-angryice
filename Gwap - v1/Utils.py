...
class Utils:
    ...
    def writeVoteData(self, user, message, images, game_session):

        f = open("vote_log.txt", "a")
        img_id = message.replace("#", "")

        des_file = open("votetag_log.txt", "r")
        des_list = des_file.read().splitlines()
        description = ""
        for line in des_list:
            fields = line.split(",")
            if(len(fields)==5):
                if (int(fields[1]) == int(img_id)):
    ...
    def get_last_game_session(self):
        game_session = 1
        with open('votetag_log.txt', 'r') as f:
            lines = f.read().rstrip('\n').splitlines()
            last_line = lines[-1]
    ...
    def check_file(self):
        if (os.path.getsize('vote_log.txt') > 0):
            return 1
        else:
            return 0

print(Utils().check_file())