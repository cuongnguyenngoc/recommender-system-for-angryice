...
class BotNotifier:
    def __init__(self, window, artManager):

        self.label = Label(window, text='Please describe pictures on the right of screen you prefer follow format "imageID:discription"', 
        bg="green", fg="white", font="none 20 bold")
        self.label.place(x=10, y=960)
    ...
    def change_label_text(self):
        try:
            des_list_file = open('descriptions_log.txt','r')
            ...
            for line in des_file_lines:
                fields = line.split(",")
                if (len(fields) == 3):
                    text = text + line.split(",")[1] + ":" + line.split(",")[2] + "\n"
        except:
    ...
    def revert_label_text(self):
        self.label.configure(text ='Please describe pictures on the right of screen you prefer follow format "imageID:discription"')
        self.artManager.refresh()

# window = tk.Tk()
# window.title("Bot Manager")
